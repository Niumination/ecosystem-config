...
**JCode status (2026-09-10)**: JCode binary and skills preserved at `~/.jcode/` (71 skills) for standalone use. JCode is NOT a sync target, NOT referenced by Hermes config, and NOT used by any ecosystem pipeline. All JCode references in active skill files have been purged.
...
```
Ekspektasi output: 2 target `✅ verifikasi hash LULUS` (semua file SHA-256 cocok).
- Hermes & USB = structure `domain` (argumen `--structure` di skill-manifest.py)
...
6. **JCode purge trap**: setelah JCode fully removed from ecosystem (2026-09-10), semua SKILL.md di `skills/ecosystem/` yang merujuk `~/.jcode/skills` sebagai target HARUS diperbarui. Jangan biarkan referensi JCode tersisa di skill bank — ini menyebabkan session berikutnya masih mencoba sync ke JCode.

## Adopsi skill eksternal → bank

Prosedur lengkap: `references/adopt-external-skill.md`
Ringkas: cek license+review → copy folder ke `skills/<domain>/<skill>/` (ikutkan references/scripts) → sesuaikan frontmatter (tambah `version`/`source`, konvensi bank) → update INDEX.md (baris tabel + counter "Status: N ✅ Aktif") → regenerate manifest → sync → verify 2 target → commit+push.
