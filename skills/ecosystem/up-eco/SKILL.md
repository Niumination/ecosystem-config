...
- **Pemberitahuan commit timestamp belum di-push (2026-09-19).** Fase Lightfix menghitung commit belum ter-push bersubjek `lightfix otomatis` dan memunculkannya sebagai `⚠️ N commit timestamp lightfix belum di-push` + rekomendasi push. Ini penutup celah kebijakan "tanpa push otomatis": commit dari cron 23:30 menumpuk lokal dan hanya terlihat sebagai `ahead`.
- **`@{upstream}` bisa fatal dan mematikan pelaporan ahead/behind (diperbaiki 2026-09-19).** Branch `main` root ecosystem-config TIDAK punya upstream tracking (`branch.main.remote` kosong), sehingga `git rev-list --count '@{upstream}'..HEAD` gagal → selalu terbaca 0 → commit menunggu push tidak pernah terlihat. `check_git_status()` dan blok pemberitahuan kini punya fallback ke `origin/<branch>`. **Repo anak (brain dll) punya upstream; root tidak — jangan asumsikan seragam.**
