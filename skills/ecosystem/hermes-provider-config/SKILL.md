...
- `scripts/check-rtk.sh` — Skrip probe cepat aktivasi RTK: binary ada? (exit 1) · rewrite sudah `rtk init -g`? (exit 2) · plugin Hermes `rtk-rewrite` ada? (exit 3) · sukses = exit 0. Dipindah dari `skill-bank-sync` saat konsolidasi bank 18 Sep 2026 (RTK bukan urusan bank skill).
