...
**Catatan fallback pasca-rekonstruksi v2 (diperbarui 21 Ags 2026):** kebijakan Niumination sekarang = **1 keluarga Zen saja** (`opencode-zen/nemotron-3-ultra-free` primary; fallback se-keluarga `hy3-free` / `nemotron-3.5-lightning-free`), bukan zoo debug (juan 401 di depan, 9router ×2). Lihat skill `niu-core-governance` (umbrella tata kelola core: hukum tersegel, kebijakan model, fence/handoff/ledger, pitfall path hook niu-*).
