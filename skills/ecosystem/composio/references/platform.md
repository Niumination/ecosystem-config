...
Both SDKs also expose `composio.create(...)`; do not teach an artificial TypeScript/Python asymmetry. The SDK reads `COMPOSIO_API_KEY` from the environment, so do not pass it inline.

`session.execute` takes the tool slug as the first positional arg and an `arguments` dict as a keyword arg. Some projects do not support multi-account selection; if execution fails with `The 'account' parameter is not supported for this project`, retry without the `account` parameter.

`session.tools()` returns a list of tool dicts. The default meta tools include `COMPOSIO_SEARCH_TOOLS`, `COMPOSIO_GET_TOOL_SCHEMAS`, `COMPOSIO_MULTI_EXECUTE_TOOL`, `COMPOSIO_MANAGE_CONNECTIONS`, and `COMPOSIO_REMOTE_BASH_TOOL`. Use `COMPOSIO_SEARCH_TOOLS` first to discover toolkit and tool slugs for a use case, then `COMPOSIO_GET_TOOL_SCHEMAS` to load the exact input schema before execution.
