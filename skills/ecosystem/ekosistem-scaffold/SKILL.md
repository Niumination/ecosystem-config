...
- **Struktur root v4.0 (Agu 2026) — `projects/`, `Production/`, `incubator/` sudah TIDAK ADA.** Proyek baru masuk `apps/` (deployed/battle-tested), `sites/` (frontend), `services/` (backend), `desktop/`, `agents/`, `labs/`, `sandbox/`, `tools/`. `eco-collect.py` sudah memindai kedelapan dir itu (`auto_discover_git_repos()`), jadi repo git baru terdeteksi otomatis tanpa menyentuh `NON_GIT_DIRS`. Baca root `AGENTS.md` (v4.0) dulu — skill ini masih menyebut struktur lama.
- **Zip pihak ketiga biasanya membungkus proyek satu level + membawa artefak mesin dev.** Ambil HANYA subdir proyek (mis. `niumination/`) berikut `.git`-nya — riwayat asli itu aset (kasus `Niu-OSS-Dashboard.zip`: 11 commit 17–18 Sep 2026). Yang dibuang: `.config/nextjs-nodejs/config.json` (telemetri Next), `uploads/`, duplikat `data/` (bandingkan `md5 -q` dulu), `*-preview.png` root, `tsconfig.tsbuildinfo` (untracked). Contoh: `rsync -a --exclude 'tsconfig.tsbuildinfo' /tmp/extract/<proyek>/ <dest>/`.
- **Cek bentrok nama repo dari README proyek SEBELUM `gh repo create`.** Nama repo GitHub case-insensitive, jadi `niumination` bentrok dengan repo profil `<Org>/<Org>` yang sudah ada. Verifikasi `gh api repos/<Org>/<nama>` (404 = bebas), lalu koreksi rujukan di README proyek dan catat koreksinya di changelog ekosistem.
- **Daftar proyek dashboard niu-dash bersumber dari script, bukan JSON.** `scripts/generate-ecosystem-json.py` menyimpan array `projects` hardcoded; `apps/niu-dash/public/data/ecosystem-status.json` hanya output (proyek baru tidak akan muncul walau `BACKLOG.md` sudah diisi). Tambahkan entri di script (`name`, `tier`, `status`, `priority`, `git`, `dox`, `desc`), jalankan ulang script, commit script + JSON.
