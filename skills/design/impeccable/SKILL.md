...
1. Run `node scripts/context.mjs` once per session (if the runtime shows this skill's loaded base directory, run `node <skill-base-dir>/scripts/context.mjs`; keep cwd at the user's project). Pass a named source file or route as `--target <path>`. It loads PRODUCT.md, DESIGN.md, the matching surface brief, and native-platform guidance when applicable; follow its directives and do not rerun it.
...
## Hermes Integration (Ekosistem Niumination)

Skill ini dijalankan di **Hermes Agent** (portable USB) — path script memakai `scripts/` relatif ke base-dir skill (sudah disesuaikan dari `.agents/skills/impeccable/scripts/`).

Workflow Hermes:
1. `node scripts/context.mjs --target <path>` — baca konteks project (PRODUCT.md, DESIGN.md, surface brief) sekali per sesi
2. Load playbook via `skill_view(name='impeccable', file_path='reference/<cmd>.md')` — mis. `reference/audit.md`, `reference/craft-floor.md`
3. Jalankan detector anti-pattern: `node scripts/detect.mjs --json <file>` — scan AI-slop dan quality issues
4. Ikuti bounded-pass verification: bangun penuh -> 1 ronde inspeksi batch (desktop+mobile) -> fix 1 batch -> maks 1 konfirmasi ronde -> stop

Catatan Hermes:
- `live` mode dan hooks butuh browser/hook infra — di Hermes gunakan detector manual (`detect.mjs`) + browser tools
- Sub-agents (Claude Code agents/) tidak dipakai di Hermes — gunakan `delegate_task` untuk parallel work
- Jangan `npx impeccable` (butuh setup Claude Code) — pakai scripts langsung via node
