...
if need_cmd hermes; then
  ...
  echo
  info "TARGET urutan (lakukan manual jika CLI add/remove interaktif):"
  info "  1. 9router / cf/@cf/deepseek-ai/deepseek-r1-distill-qwen-32b"
  info "  2. 9router / gratislonggar"
  info "  3. huancheng / DeepSeek-V4-Flash     (HANYA jika probe 200 + auth)"
  info "  4. juan-router / agnes-2.0-flash     (HANYA jika probe 200 + auth)"
  info "JANGAN biarkan juan-router di posisi 1 selama HTTP 401."
else
...
ok "orca-status        : tunda sampai audit overlap watchdog×5"
echo
