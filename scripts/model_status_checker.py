...
Tier 2: Minimal probe for gap models  
...
def main():
    ...
    report["providers"]["openrouter"] = {
        "total": len(or_models),
        "free": len(or_free),
        "free_models": [{"id": m["id"], "name": m["name"]} for m in or_free]
    }
    ...
    report["providers"]["9router"] = {
        "total": len(r9_models),
        "namespaces": {k: len(v) for k, v in r9_ns.items()},
        "sample": [m["id"] for m in r9_models[:10]]
    }
    ...
    for model_id, provider in critical_models:
        ...
        if model_id in INTERNAL_ROUTE_MODELS:
            info = INTERNAL_ROUTE_MODELS[model_id]
    ...
    ok_critical = [m for m, r in probe_results.items() if r["status"] in ("ok", "internal_route")]
    failed_critical = [m for m, r in probe_results.items() if r["status"] not in ("ok", "internal_route", "internal_namespace", "no_key", "no_base_url")]
    skipped = [m for m, r in probe_results.items() if r["status"] in ("internal_namespace", "no_key", "no_base_url")]
    active_internal = [m for m, r in probe_results.items() if r["status"] == "internal_route"]
    
    report["summary"] = {
        "critical_ok": len(ok_critical),
        "critical_failed": len(failed_critical),
        "critical_skipped": len(skipped),
        "critical_active_internal": len(active_internal),
        "openrouter_free_available": len(or_free),
        "9router_total": len(r9_models)
    }
    ...
    cache_data = {
        "timestamp": now.isoformat(),
        "expires": (now + timedelta(hours=CACHE_TTL)).isoformat(),
        "report": report
    }
    ...
    print()
    print(f"✅ Critical OK: {len(ok_critical)} (includes {len(active_internal)} via internal route)")
    if failed_critical:
